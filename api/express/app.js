...
const routes = {
	adminData: require('./routes/adminData'),
	companyData: require('./routes/companyData'),
	industryData: require('./routes/industryData'),
	feedbackData: require('./routes/feedbackData'),
...
app.get('/', (req, res) => {
	res.send(`
		...
		<p>Try some routes, such as <a href='/api/adminData'>/api/adminData</a> or <a href='/api/orchestras?includeInstruments'>/api/orchestras?includeInstruments</a>!</p>
...
for (const [routeName, routeController] of Object.entries(routes)) {
	...
	if (routeController.getByTitle) {
		app.get(
			`/api/${routeName}/0/:title`,
			makeHandlerAwareOfAsyncErrors(routeController.getByTitle)
		);
	}
